...
export type { EntityId, AgentInput, AgentOutput, AgentContext } from '@confused-ai/core';
export { AgentState } from '@confused-ai/core';
// Also export the public Agent interface (has .id, .name, .run(prompt))
export type { Agent } from '@confused-ai/core';

// Local imports for use within this file
import type { EntityId, AgentInput, AgentOutput } from '@confused-ai/core';

/**
 * Orchestration-internal agent interface.
 *
 * This variant of Agent uses the contracts-level `run(input, ctx)` signature
 * so orchestrators can pass rich execution context (memory, tools, planner).
 * Use `createRunnableAgent()` to wrap a function into one of these.
 */
export interface OrchestrableAgent {
    readonly id: EntityId;
    readonly name: string;
    readonly description?: string;
    run(input: AgentInput, ctx: import('@confused-ai/core').AgentContext): Promise<AgentOutput>;
}
...
export interface Orchestrator {
     ...
     */
    registerAgent(agent: OrchestrableAgent, role: AgentRole): Promise<void>;
