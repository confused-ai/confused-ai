...
class MinHeap {
    ...
    pop(): HeapNode {
        // data[0] is guaranteed to exist when pop() is called (heap is non-empty)
        const top = this.data[0] as HeapNode;
    ...
    private _bubbleUp(i: number): void {
        while (i > 0) {
            const parent = (i - 1) >> 1;
            if ((this.data[parent] as HeapNode).freq <= (this.data[i] as HeapNode).freq) break;
            [this.data[parent], this.data[i]] = [this.data[i] as HeapNode, this.data[parent] as HeapNode];
            i = parent;
        }
    }
    ...
    private _sinkDown(i: number): void {
        ...
        while (true) {
            ...
            const l = 2 * i + 1;
            const r = 2 * i + 2;
            if (l < n && (this.data[l] as HeapNode).freq < (this.data[smallest] as HeapNode).freq) smallest = l;
            if (r < n && (this.data[r] as HeapNode).freq < (this.data[smallest] as HeapNode).freq) smallest = r;
            if (smallest === i) break;
            [this.data[i], this.data[smallest]] = [this.data[smallest] as HeapNode, this.data[i] as HeapNode];
            i = smallest;
        }
    }
}
...
function buildTable(freq: Map<number, number>): HuffmanTable {
    ...
    if (freq.size === 1) {
        const [sym] = freq.keys();
        // sym is guaranteed here since freq.size === 1
        const safeSym = sym as number;
        const table: HuffmanTable = { encode: new Map(), decode: new Map() };
        table.encode.set(safeSym, '0');
        table.decode.set('0', safeSym);
        return table;
    }
    ...
    const traverse = (node: HeapNode, depth: number): void => {
        ...
        if (node.left !== undefined) traverse(node.left, depth + 1);
        if (node.right !== undefined) traverse(node.right, depth + 1);
    };
    traverse(heap.pop(), 0);
...
export class HuffmanCodec {
    ...
    encode(text: string): HuffmanEncodeResult {
        ...
        for (let i = 0; i < data.length; i++) {
            const byte = data[i] ?? 0;
            const code = this.table.encode.get(byte);
            if (code === undefined) throw new Error(`Symbol ${byte} not in table`);
            writer.write(code);
        }
...
export function compressContext(text: string): string {
    ...
    return `H1:${String(result.totalBits)}:${tableB64}:${dataB64}`;
}
...
export function decompressContext(compressed: string): string {
    ...
    const totalBits = parseInt(parts[1] ?? '0', 10);
